...
struct WeightChartView: View {
    ...
    private var slotData: [(date: Date, weight: Double?)] {
        rollingWindowDays.map { day in
            (date: day, weight: weightByDay[day])
        }
    }

    private var plottedPoints: [(slotIndex: Int, date: Date, weight: Double)] {
        slotData.enumerated().compactMap { index, slot in
            guard let weight = slot.weight else { return nil }
            return (slotIndex: index, date: slot.date, weight: weight)
        }
    }
